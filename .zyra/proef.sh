...
# geen eigen host-app).
#
# Dit package heeft zelf geen artisan/.env/routes/serve-entrypoint, want
# het is bedoeld om in een host-Laravel-app geïnstalleerd te worden en
# leunt op `ginkelsoft/laravel-compliance-core` voor gedeelde primitieven
# (subject hashing, hash chain, anonymize-strategieën). Voor lokaal
# proefdraaien gebruiken we daarom Orchestra Testbench: dat start een
# minimale Laravel-skeleton-app met deze package erin geladen, zodat je
# zonder aparte host-app kunt rondkijken en de testsuite kunt draaien.
#
# Gebruik:
